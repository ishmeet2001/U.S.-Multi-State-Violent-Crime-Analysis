...
OUTPUT_FILENAME = 'new_mexico_2024.csv' 
...
def merge_comprehensive_summary():
    ...
    join_key = 'offense_type_id' if 'offense_type_id' in main_df.columns else 'offense_code'
    offense_type_cols = ['offense_name', 'offense_category_name', join_key]

    main_df = pd.merge(main_df, lookup_data['offense_type'][offense_type_cols], on=join_key, how='left')
    
    main_df = pd.merge(main_df, lookup_data['location_type'][['location_id', 'location_name']], on='location_id', how='left')

    victim_df_renamed = core_data['victim'].rename(columns={
    ...
    main_df = pd.merge(main_df, victim_link_df, on=['offense_id', 'incident_id'], how='left')
